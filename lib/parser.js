 ...
 * @param string input
 */
ShortcodeParser.prototype.parse = async function (input) {
  var self = this;
...
ShortcodeParser.parsers.error = function (t, ctx) {
  if (ctx && ctx.name) {
    if (ctx.name) t.toBuffer(this.options.openPattern.replace(/\\/g, "") + "!" + ctx.name);

    if (ctx.errorUnclosed) {
      console.error('[Shortcode Parser] Tag unclosed', ctx.name);
      t.toBuffer("!" + this.options.closePattern.replace(/\\/g, "") + ctx.content);
    } else if (ctx.content !== "") {
      console.error('[Shortcode Parser] No content', ctx.name);
      t.toBuffer(
        "!" +
      ...
      console.error('[Shortcode Parser] Other error', ctx.name);
      
      t.toBuffer("!/" + this.options.closePattern.replace(/\\/g, ""));
    }
  } else {
    console.error('[Shortcode Parser] No tag name error', ctx.name);
...
ShortcodeParser.parsers.tagName = function (t, ctx) {
  if (!t.match("[a-zA-Z0-9\\-_]+", false, true)) return ShortcodeParser.parsers.error;
...
ShortcodeParser.parsers.params = function (t, ctx) {
  var addParam = function () {
    if (t.getBuffer().trim() !== "") {
      if (ctx.argKey) {
        ctx.args[ctx.argKey] = t.clearBuffer() || undefined;
      } else {
        const v = t.clearBuffer();
        ctx.args[v] = v;
      }
    }
    // if (t.getBuffer().trim() !== "") {
    //   if (!ctx.argKey && !Array.isArray(ctx.args) && Object.keys(ctx.args).length === 0) {
    //     ctx.args = [];
    //   }
    //   if (ctx.argKey) {
    //     const v = {};
    //     v[ctx.argKey] = t.clearBuffer();
    //     ctx.args.push(v);
    //   } else {
...
ShortcodeParser.parsers.process = async function (t, ctx) {
  ...
  const shortcodeOutput = await this.shortcodes[ctx.name].call(ctx, ctx.args, ctx.content);
